...
import type { TuiRound, TuiState } from "../../src/state/types.js";
...
describe("rebuildChunks", () => {
	...
	it("emits separate judge chunk after completed round with judge result", () => {
		const state = makeMinimalState();
		state.rounds = [
			{
				roundNumber: 1,
		...
		expect(chunks[0].type).toBe("round");
		expect(chunks[1].type).toBe("judge");
		if (chunks[1].type === "judge") {
			expect(chunks[1].roundNumber).toBe(1);
...
describe("rebuildChunks — collapsed round summary", () => {
	...
	it("collapsed round emits separate judge chunk (not embedded)", () => {
		const state = makeMinimalState();
		state.rounds = [
			{
				roundNumber: 1,
		...
		expect(judgeChunks).toHaveLength(1);
		if (judgeChunks[0].type === "judge") {
			expect(judgeChunks[0].roundNumber).toBe(1);
			expect(judgeChunks[0].shouldContinue).toBe(true);
		}
	});
});

describe("rebuildChunks — judge as separate chunk", () => {
	it("expanded round with judge result emits round + judge chunks", () => {
		const state = makeMinimalState();
		state.rounds = [
			{
				roundNumber: 1,
